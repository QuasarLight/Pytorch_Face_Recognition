...
def loadModel(data_root, file_list, backbone_net, gpus='0', model_para_path=None):
    ...
    lfw_dataset = LFW(data_root, file_list)
    lfw_loader = DataLoader(lfw_dataset, batch_size=128, shuffle=False, num_workers=4, drop_last=False)
...
if __name__ == '__main__':
    ...
    net, device, agedb_dataset, agedb_loader = loadModel(args.lfw_dataset_path, args.lfw_file_list, args.backbone, args.gpus, model_para_path)
    getFeatureFromTorch('Test_Data/cur_lfw_result.mat', net, device, agedb_dataset, agedb_loader)
    ACCs = evaluation_10_fold('Test_Data/cur_lfw_result.mat')
